...
class TestAddAndSearchDocuments:
    ...
    def test_delete_document_by_source(
        ...
        deleted = store.delete_document("/docs/overview.md")
        assert deleted == 2

    def test_delete_nonexistent_document_returns_zero(
        self, store: VectorStore
    ) -> None:
        deleted = store.delete_document("/nonexistent/file.md")
        assert deleted == 0
...
class TestConversations:
    def test_add_conversation_returns_id(self, store: VectorStore) -> None:
        conv_id = store.add_conversation("Hello, how can I help?")
    ...
    def test_delete_conversation_true_for_existing(
        ...
        conv_id = store.add_conversation("Test conversation", {"user_id": "u1"})
        assert store.delete_conversation(conv_id) is True

    def test_delete_conversation_false_for_nonexistent(
        self, store: VectorStore
    ) -> None:
        assert store.delete_conversation("00000000-0000-0000-0000-000000000000") is False
